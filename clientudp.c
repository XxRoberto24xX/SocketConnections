...
char *argv[];
{
   ...
   /* Registrar SIGALRM para no quedar bloqueados en los recvfrom */
	...
	while (n_retry > 0) {
        ...
        printf("mando mensaje de conexion\n");
        if (sendto (s, "\r\n", strlen("\r\n"), 0, (struct sockaddr *)&servaddr_in, sizeof(struct sockaddr_in)) == -1) {
        	perror("clientUDP");
        ...
        if (( len = recvfrom (s, buf, BUFFERSIZE, 0, (struct sockaddr *)&servaddr_in, &addrlen))== -1) {
    		if (errno == EINTR) {
    ...
    if(argc == 3){
        ...
        if(file != NULL){
            ...
            while(fgets(linea, sizeof linea, file) != NULL){
                printf("%s", linea);
                strcpy(buf, linea);
                if (sendto (s, buf, BUFFERSIZE, 0, (struct sockaddr *)&servaddr_in, sizeof(struct sockaddr_in)) == -1) {
                    perror("clientUDP");
                    fprintf(stderr, "clientUDP: unable to send request\n");
                    exit(1);
                }else{
